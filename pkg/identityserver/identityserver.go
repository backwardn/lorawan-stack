...
func New(c *component.Component, config *Config) (is *IdentityServer, err error) {
	...
	for _, hook := range []struct {
		...
		hooks.RegisterUnaryHook("/ttn.lorawan.v3.ClusterRegistry", hook.name, hook.middleware)
		hooks.RegisterUnaryHook("/ttn.lorawan.v3.ClusterAccess", hook.name, hook.middleware)
...
func (is *IdentityServer) RegisterServices(s *grpc.Server) {
	...
	ttnpb.RegisterClusterRegistryServer(s, &clusterRegistry{IdentityServer: is})
	ttnpb.RegisterClusterAccessServer(s, &clusterAccess{IdentityServer: is})
...
func (is *IdentityServer) RegisterHandlers(s *runtime.ServeMux, conn *grpc.ClientConn) {
	...
	ttnpb.RegisterClusterRegistryHandler(is.Context(), s, conn)
	ttnpb.RegisterClusterAccessHandler(is.Context(), s, conn)
